 ...
 * @package Blog Rich
 */
$blog_rich_blog_style = get_theme_mod('blog_rich_blog_style', 'classic');
...
elseif ($blog_rich_blog_style == 'grid' && !is_single()) :
	...
	$blog_rich_cat_list = get_the_category_list(esc_html__(' ', 'blog-rich'));
	$blog_rich_tags_list = get_the_tag_list(sprintf('<p class="tags-list">%s: ', __('Tags', 'blog-rich')), ' ', '</p>');
				...
				<div class="rich-list-right">
					...
					<div class="entry-content <?php if (!is_single()) : ?>rich-blog-desk-only<?php endif; ?>">
						...
						the_content(
							sprintf(
								wp_kses(
									...
									__('Continue reading<span class="screen-reader-text"> "%s"</span>', 'blog-rich'),
									array(
										'span' => array(
											'class' => array(),
										),
						...
						if (is_single()) {
							wp_link_pages(
								array(
									'before' => '<div class="page-links">' . esc_html__('Pages:', 'blog-rich'),
									'after'  => '</div>',
								)
							);
						}
