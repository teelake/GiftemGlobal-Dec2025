...
jQuery(document).ready(function(){
	...
	if(softaculous_pro_setup_info && softaculous_pro_setup_info.type){
		...
		$('#cat_button_'+softaculous_pro_setup_info.type).trigger("click");
	}
});
...
function softaculous_pro_update_cat_input(other_cat){
	$('#cat_input').val(other_cat);
}

function softaculous_pro_templates_fn($){

// Back button handler
$('.softaculous-pro-back').click(function(){
	softaculous_pro_show_themes(softaculous_pro_setup_info.type !== undefined ? softaculous_pro_setup_info.type.toLowerCase() : '');
	$("#spro_import_content").hide();
	$(".softaculous-pro-back").hide();
});

$('.softaculous-pro-back-theme').click(function(){
	$('#softaculous-pro-templates-holder').show();
	$(this).parent().hide();
	$('#SproTemplatesModal').hide();
});

$('#cat_input').keyup(function() {
	
	var query = $(this).val().toLowerCase();
	var cat_displayed = 0;
	
	$(".category_btn").each(function( index ){
		var cslug = $(this).find("input").attr("data-target");
		...
		if(cat_displayed > 0){
			$("#spro_no_cat_results").hide();
		}else{
			$("#spro_no_cat_results").show();
		}
	});
...
$('.softaculous-pro-cat-holder').html(chtml);
$('.softaculous-pro-cat-holder').find('.softaculous-pro-cat').click(function(){
	softaculous_pro_show_themes($(this).data('cat'));
});

$('.softaculous-pro-categories-list').find('.category_btn').click(function(){
	var childEle = $(this).children('input');
	var jEle_parent =  $(this).parent().find(".active_category");
	var real_val = $(this).children('input').val();
	var val = $(this).children('input').val().toLowerCase();
	var inputSection = $(".softaculous-pro-category-input");
...
// Search Clear
$('.softaculous-pro-sf-empty').click(function(){
	$('.softaculous-pro-search-field').val('');
	softaculous_pro_show_themes();
});

// Search
$('.softaculous-pro-search-field').on('keyup', function(e){
	softaculous_pro_show_themes('', $(this).val());
});

// Sort themes
$('.softaculous-pro-sortby').change(function(){
	softaculous_pro_show_themes($('.softaculous-pro-current-cat').data('cat'), $('.softaculous-pro-search-field').val());
});
...
function softaculous_pro_show_themes(cat, search, page){
	...
	var sortby = 'latest';
	$("#softaculous_pro_search").show();	
	$("#softaculous_pro_theme_title").show();	
	$("#softaculous-pro-suggestion").hide();	
	$("#softaculous-pro-single-template").hide();
	$("#softaculous-pro-pagination").show();
	$("#softaculous-pro-templates").show();
	
	// Blank html
	$('#softaculous-pro-templates').html('');
	$('#softaculous-pro-pagination').html('');
	...
	if(cat.length > 0){
		$('.softaculous-pro-current-cat').html(cat_appender);
		$('.softaculous-pro-current-cat').data('cat', cat);
	}else{
		$('.softaculous-pro-current-cat').html('All');
		$('.softaculous-pro-current-cat').data('cat', '');
	}
	...
	$('.softaculous-pro-theme-details').click(function(){
		var jEle = $(this);
	...
	if(pages > 1){
		...
		$('#softaculous-pro-pagination').html(html);
		
		$('#softaculous-pro-pagination').find('.page-link').click(function(){
			var j = $(this);
...
function softaculous_pro_show_theme_tile(theme, x){
	var html = '<div class="softaculous-pro-md-4">'+
		...
		'</div>'+
	'</div>';
	$('#softaculous-pro-templates').append(html);
}
...
function softaculous_pro_show_theme_details(slug){
	...
	$("#softaculous-pro-suggestion").hide();	
	$("#softaculous_pro_search").hide();	
	$("#softaculous_pro_theme_title").hide();	
	$("#softaculous-pro-single-template").show();
	$("#softaculous-pro-pagination").hide();
	$("#softaculous-pro-templates").hide();
	
	// Set install value
	$('#softaculous-pro-template-install').val(slug);
			
	// Set name
	$("#softaculous-pro-template-name").html(theme['name']);
			
	// Demo URL
	$("#softaculous-pro-demo").attr("href", softaculous_pro_demo+(theme['name'].replace(' ', '_')));
	
	// Blank screenshots
	$("#softaculous_pro_screenshots").html('');
	
	// Is the license PRO ?
	if(theme['type'] >= 2){
		$('#softaculous_pro_license_div').css('display', 'inline-block');
	}else{
		$('#softaculous_pro_license_div').hide();
	}
	
	var url = mirror+'/'+theme['slug'];
	
	// Show home image
	$("#softaculous_pro_display_image").attr("src", "");
	$("#softaculous_pro_display_image").attr("src", url+'/screenshots/home.jpg');
	$("#softaculous_pro_display_image").parent().scrollTop(0);
	...
	jQuery.ajax({
		...
		success:function(theme) {
			...
			$("#spro_import_single").addClass("hidden");
			...
			$("#softaculous_pro_screenshots").html(sc);
			$("#spro_import_content").show();
			$(".softaculous-pro-back").show();
			$('.softaculous_pro_img_screen:first').children('a').addClass('spro_img_inview');


			$("#softaculous_pro_screenshots").find('.softaculous_pro_img_views').click(function(e){
				e.preventDefault();
				var jEle = $(this);
				$("#softaculous_pro_display_image").hide();
				$(".loader").show();
				
				if($('.softaculous_pro_img_screen .softaculous_pro_img_views').hasClass('spro_img_inview')){
					$('.softaculous_pro_img_screen .softaculous_pro_img_views').removeClass('spro_img_inview');
				}
				
				var newImageSrc = jEle.attr("href");
				$("#softaculous_pro_display_image").attr("src", newImageSrc);

				// Handle image load event
				$("#softaculous_pro_display_image").on('load', function() {
					$(".loader").hide(); // Hide loader
					$(this).show(); // Show image
				});

				// In case the image is cached and loads immediately
				if ($("#softaculous_pro_display_image")[0].complete) {
					$(".loader").hide(); // Hide loader
					$("#softaculous_pro_display_image").show(); // Show image
				}
			...
			$("#softaculous_pro_screenshots").find('.softaculous_pro_img_views').on('mouseenter',function(e){
				var imgUrl = $(this).attr('href');
				if(!$(this).attr('loaded')){
					$('<img>').attr('src', imgUrl).on('load', function() {}).appendTo('body').css('display', 'none');
				}
				$(this).attr('loaded',true);
			});

			$("#softaculous_pro_screenshots").find('.spro_page_selector').click(function(event){
				var jEle = $(this);
				$("#softaculous_pro_display_image").hide();
				$(".loader").show();

				var checkbox = $(this).find('.checkbox');
				if ($(event.target).is('.checkbox') || $(event.target).is('.softaculous_pro_img_name')) {
					return;
				}
			 ...
			 $("#softaculous_pro_screenshots").find('.checkbox').change(function() {
				var checked_div = $(this).siblings('.softaculous_pro_img_name');

				if($('.softaculous_pro_img_screen .softaculous_pro_img_views').hasClass('spro_img_inview')){
					$('.softaculous_pro_img_screen .softaculous_pro_img_views').removeClass('spro_img_inview');
				}
				var newImageSrc = $(this).parent().siblings('.softaculous_pro_img_views').attr("href");

				$(this).parent().siblings('.softaculous_pro_img_views').addClass('spro_img_inview');
				$("#softaculous_pro_display_image").attr("src", newImageSrc);
