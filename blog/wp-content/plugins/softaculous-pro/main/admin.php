...
function softaculous_pro_page_settings_theme(){
	...
	$(".spro-tour-btn").click(function(){
		$(this).find("i").removeClass("fa-play");
		$(this).find("i").addClass("fa-rotate-right");
		var spro_tour_page = $(this).attr("spro-tour-id");
	...
	$(document).ready(function() {
		...
		$(".spro-settings input").each(function(){
			$(this).click(function(){
				var requires_reload = $(this).attr("requires-reload");
				var data = {
	...
	function spro_highlight(box){
		...
		$("#spro-"+box).css({"box-shadow": "rgba(255, 33, 33, 0.8) 0px 0px 1px 2px, rgba(33, 33, 33, 0.5) 0px 0px 0px 0px"});
		setTimeout(function () {
			$("#spro-"+box).css({"box-shadow": "none"});
        }, 1500);
	}
...
function softaculous_pro_assistant(){
	...
	if(!empty($_COOKIE['spro-load-tour']) && (!empty($spro_tours)) && array_key_exists($_COOKIE['spro-load-tour'], $spro_tours) && $current_page == $spro_tours[$_COOKIE['spro-load-tour']]){
		$spro_load_tour = $_COOKIE['spro-load-tour'];
	}
...
function softaculous_pro_assistant_theme(){
	...
	function spro_can_init_introjs(){
		...
		if(spro_load_tour != "themes" || $(".add-new-theme").length > 0 || spro_tour_retries >= 600){
			spro_init_introjs();
			return true;
		}
